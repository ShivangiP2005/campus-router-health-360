...
def diagnose_router(router_id: str, scored_df: pd.DataFrame,
    ...
    if is_healthy and has_complaints:
        return {
            ...
            "evidence": {
                ...
                "avg_packet_loss_pct": round(float(row["avg_packet_loss_pct"]), 2),
                "bad_hour_count": int(row["bad_hour_count"]),
                "complaint_count": int(len(my_complaints)),
                "sample_complaint": str(my_complaints.iloc[0]["complaint_text"]),
            },
            "fix": "user_education",
            "confidence": 0.7,
        }
        ...
        if not fw_row.empty and fw_row.iloc[0]["affected_router_count"] >= 3 \
            ...
            return {
                ...
                "evidence": {
                    ...
                    "fleet_avg_bad_hour_count_on_firmware": round(float(fw_row.iloc[0]["avg_bad_hour_count"]), 2),
    ...
    if row["avg_signal_dbm"] < SIGNAL_WEAK_DBM:
        return {
            ...
            "evidence": {
                "avg_signal_dbm": round(float(row["avg_signal_dbm"]), 2),
    ...
    if row["sustained_bad"]:
        return {
            ...
            "evidence": {
                ...
                "avg_latency_ms": round(float(row["avg_latency_ms"]), 2),
                "avg_packet_loss_pct": round(float(row["avg_packet_loss_pct"]), 2),
                "avg_disconnects": round(float(row["avg_disconnects"]), 2),
